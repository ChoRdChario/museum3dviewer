...
function getFileThumbUrl(fileId, token, size){
  ...
  return window.__lm_fetchDrive(url)
...
function getValues(spreadsheetId, rangeA1, token){
  ...
  return window.__lm_fetchDrive(url).then(r=>{ if(!r.ok) throw new Error('values.get '+r.status); return r.json(); }).then(d=> d.values||[]);
}
function isLociMyuSpreadsheet(spreadsheetId, token){
  ...
  return window.__lm_fetchDrive(url).then(res=> res.ok ? res.json() : false).then(data=>{
...
function createLociMyuSpreadsheet(parentFolderId, token, opts){
  ...
  return window.__lm_fetchDrive(url, {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(body)}).then(r=>{ if(!r.ok) throw new Error('Drive files.create '+r.status); return r.json(); }).then(file=>{
...
function findOrCreateLociMyuSpreadsheet(parentFolderId, token, opts){
  ...
  return window.__lm_fetchDrive(url).then(r=>{ if(!r.ok) throw new Error('Drive list spreadsheets '+r.status); return r.json(); }).then(d=>{
      const files = d.files || [];
      function next(i){
...
async function deleteCaptionRowFromSheet(spreadsheetId, sheetId, rowIndex1based, token){
  try{
    ...
    const r = await window.__lm_fetchDrive(url, {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(body)});
    if(!r.ok){
...
function populateSheetTabs(spreadsheetId, token){
  ...
  return window.__lm_fetchDrive(url).then(function(r){ return r.ok ? r.json() : null; }).then(function(data){
...
if(btnCreate){
  btnCreate.addEventListener('click', function(){
    ...
    window.__lm_fetchDrive(url, {method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(body)})
...
(function(){
  ...
  async function GV(ssid, rangeA1, token){
    ...
    const r = await window.__lm_fetchDrive(url);
