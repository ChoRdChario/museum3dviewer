...
function __normCanvas(cnv){
  if (!cnv) return null;
  if (typeof cnv === 'string') return document.getElementById(cnv.replace(/^#/, '')) || document.querySelector(cnv);
  return cnv; // assume Element
}
...
try {
  ...
  try {
    ...
    try { if (window.lm && typeof window.lm.__resolveReadyScene==='function') window.lm.__resolveReadyScene(scene); } catch(e) { console.warn('[viewer] deep-ready resolve failed', e); }
    try { window.dispatchEvent(new CustomEvent('pm:scene-deep-ready', {detail:{scene}})); } catch(e) {}
...
// explicit named exports for boot.esm.cdn.js compatibility
export { getScene, loadGlbFromDrive };
