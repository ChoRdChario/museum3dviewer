...
/* materials.module.js — ensure headers + append + simple render bridge */
(function(){
  ...
  async function appendRow(rec, attempt=0){
    ...
    try{
      ...
      if(!res.ok){ const t=await res.text().catch(()=>"(no body)"); console.warn("[materials] append url="+url+" status="+res.status, t);
        if((res.status===429||res.status>=500)&&attempt<4){ const wait=400*(2**attempt); await new Promise(r=>setTimeout(r,wait)); return appendRow(rec, attempt+1); }
  ...
  function init(){
    ...
    console.log("[materials.mod] ready");
  }
