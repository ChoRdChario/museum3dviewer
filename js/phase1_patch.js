...
// --- Helper: apply color key shader to a material (Standard/Physical/Basic supported) ---
function applyColorKeyToMaterial(mat, cfg){
  ...
  // Remove previous hook if any (will replace below)
  if(!mat.userData) mat.userData = {};

  if(!wantKey){
    if(mat.userData.__lmy_restore){
      try { mat.onBeforeCompile = mat.userData.__lmy_restore; } catch(_){}
    }else{
...
// Traverse scene and apply material rules
...
// --- Patch 1: Camera preset remap ---
(function(){
  const proto = (window.viewer && window.viewer.__proto__) || (window.Viewer && window.Viewer.prototype);
  if(!proto) return;
...
// --- Patch 2: Ortho zoom via mouse wheel ---
(function(){
  const proto = (window.viewer && window.viewer.__proto__) || (window.Viewer && window.Viewer.prototype);
  if(!proto) return;
...
// --- Patch 3: Material config – fix white/black key & transparency model ---
(function(){
  const proto = (window.viewer && window.viewer.__proto__) || (window.Viewer && window.Viewer.prototype);
  if(!proto) return;
...
console.log('[LociMyu] Phase1 patch loaded');
