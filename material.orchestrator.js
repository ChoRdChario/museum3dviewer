 ...
 * - GISトークン取得: silent→consent の順でフォールバック
 * - 現在のスプレッドシート文脈へ自動バインド（イベント/スニッフ両対応）
 * - 初回使用時に __LM_MATERIALS を自動生成
 * - 既存API互換:
...
(() => {
  ...
  async function getAccessToken(){
    ...
    try {
      if (typeof window.__lm_getAccessToken==='function'){
        const tok = await window.__lm_getAccessToken(); if (tok) return tok;
      }
    } catch {}
    // 2) 旧gauth
    try {
      if (window.gauth?.getAccessToken){
        const tok = await window.gauth.getAccessToken(); if (tok) return tok;
      }
    } catch {}
    // 3) GIS
  ...
  async function upsertProps({ matUuid, matName, props }){
    ...
    if (rowIndex>0){
      ...
      const putRes = await authFetch(
        ...
        { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ values:[payload] }) }
      );
  ...
  // ---------- UI hooks ----------
  function captureUI(){
    state.ui.dropdown     = document.querySelector('#pm-material') ||
                            document.querySelector('#lm-material-select');
    state.ui.perMatSlider = document.querySelector('#pm-opacity-range') ||
                            document.querySelector('input[type="range"][data-lm="permat-opacity"]');
  }
  ...
  function sniffSheetContext(){
    ...
    for (const f of cands){
      try { const v=f(); if (v?.spreadsheetId) return v; } catch {}
    }
  ...
  async function ensureIfCtx(){
    ...
    if (r.spreadsheetId && (r.sheetGid ?? '')!==null){
      try { const res = await window.lmMaterials.ensureMaterialSheet(); log('auto-ensure', res); }
      catch(e){ warn('auto-ensure failed', e); }
    } else {
