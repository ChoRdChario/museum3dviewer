...
const HEADER = ['id','x','y','z','title','body','imageUrl','createdAt','updatedAt'];
...
export function createAutosave({ bus, store, findOrCreateSpreadsheetInSameFolder, glbFileId }){
  ...
  async function ensureSheet(){
    spreadsheetId = await findOrCreateSpreadsheetInSameFolder(glbFileId);
    await ensureHeader(spreadsheetId, SHEET_NAME, HEADER);
  }
  ...
  async function flush(){
    ...
    await gapi.client.sheets.spreadsheets.values.clear({
      spreadsheetId, range: `${SHEET_NAME}!A2:Z`
    });
    if (rows.length){
