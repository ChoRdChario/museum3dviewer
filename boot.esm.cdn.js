...
function putValues(spreadsheetId, rangeA1, values, token){
  ...
  return fetch(url, { method:'PUT', headers:{ Authorization:'Bearer '+token, 'Content-Type':'application/json' }, body: JSON.stringify({ values, majorDimension:'ROWS' }) })
...
function appendValues(spreadsheetId, rangeA1, values, token){
  ...
  return fetch(url, { method:'POST', headers:{ Authorization:'Bearer '+token, 'Content-Type':'application/json' }, body: JSON.stringify({ values, majorDimension:'ROWS' }) })
...
function sheetsAppendRow(spreadsheetId, sheetTitle, obj){
  ...
  return enqueueSave(()=> appendValues(spreadsheetId, "'"+sheetTitle+"'!A:Z", vals, token)).then(()=> ensureIndex());
}
...
function updateCaptionForPin(id, fields){
  ...
  return ensureRow(id, seed).then(()=> ensureIndex()).then(()=>{
    ...
    if(!meta && currentSpreadsheetId){
      ...
      const values = lmNormalizeRow(seed);
...
// ---------- Save queue (serialize writes per sheet) ----------
let __saveQueue = Promise.resolve();
function enqueueSave(fn){
  __saveQueue = __saveQueue.then(()=>fn()).catch(e=>{ console.warn('[save]', e); });
  return __saveQueue;
}

function lmNormalizeRow(obj){
  const headers = LOCIMYU_HEADERS;
  ...
  if(out.length > headers.length) out.length = headers.length;
  return out;
}
=