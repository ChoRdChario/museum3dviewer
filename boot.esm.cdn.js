...
// ===== end LM-PATCH-A6 =====
// === LM-MATERIALS-PATCH-START (stable, single-flight, backoff) ===
(function(){
  const TAG = '[materials]';
  const VERSION = 'LM-PATCH-STABLE-1';
  try { console.log(`${TAG} overlay applied ${VERSION}`); } catch(e){}
  ...
  async function ensureAuth(){
    if(typeof window.ensureToken === 'function'){ try{ await window.ensureToken(); }catch(e){} }
  ...
  async function ensureMaterialsSheet(token, ssid){
    const headers = ['sheetId','materialKey','unlit','doubleSided','opacity','white2alpha','whiteThr','black2alpha','blackThr','updatedAt','updatedBy'];
    try{
      ...
      if(!r.ok){
        ...
        console.warn(TAG, 'addSheet fail', r.status, t);
      }
      try{ await PV(ssid, `${MATERIALS_SHEET_TITLE}!A1:K1`, [headers], token); }catch(e2){ console.warn(TAG, 'put header fail', e2); }
      return true;
    }
  }
  ...
  const idx = new Map();
  const cache = new Map();
  const keyOf = (sheetId, mkey)=> `${sheetId}::${mkey}`;

  async function loadIndex(token, ssid){
    idx.clear(); cache.clear();
    try{
      const values = await GV(ssid, `${MATERIALS_SHEET_TITLE}!A1:K9999`, token);
      if(!values || !values.length) return;
      const headers = values[0].map(v => (v||'').toString().trim());
      const col = {}; headers.forEach((h,i)=> col[h.toLowerCase()] = i);
      const iSheetId = col['sheetid'], iKey = col['materialkey'];
      for(let r=1;r<values.length;r++){
        const row = values[r]||[];
        const s = Number(row[iSheetId]||0);
        const mk = (row[iKey]||'').toString();
        if(!s || !mk) continue;
        const k = keyOf(s, mk); idx.set(k, r+1);
        const getB = (name, def)=>{ const i = col[name]; if(i==null) return def; const v = String(row[i] ?? '').toLowerCase(); return v==='1'||v==='true'; };
        const getN = (name, def)=>{ const i = col[name]; if(i==null) return def; const n = Number(row[i]); return isFinite(n)? n : def; };
        cache.set(k, { unlit:getB('unlit',false), doubleSided:getB('doublesided',false), opacity:getN('opacity',1),
    ...
    }catch(e){ console.warn(TAG, 'loadIndex', e); }
  }

  let saveTimer = null, inFlight = false, pending = null;

  async function saveNow(sheetId, materialKey, settings){
    const token = await ensureAuth(); if(!token) return;
    const ssid = window.currentSpreadsheetId; if(!ssid) return;
    await ensureMaterialsSheet(token, ssid);

    const now = new Date().toISOString();
    const user = (window.gapiUserEmail || 'unknown');
    const row = [ sheetId, materialKey, settings.unlit?1:0, settings.doubleSided?1:0, settings.opacity,
      settings.white2alpha?1:0, settings.whiteThr, settings.black2alpha?1:0, settings.blackThr, now, user ];

    const key = keyOf(sheetId, materialKey);
    const rowIndex = idx.get(key);
    const range = rowIndex ? `${MATERIALS_SHEET_TITLE}!A${rowIndex}:K${rowIndex}` : `${MATERIALS_SHEET_TITLE}!A2:K9999`;

    let delay = 250;
    for(let attempt=0; attempt<6; attempt++){
      try{
        if(rowIndex){ await PV(ssid, range, [row], token); }
        else{
          await AV(ssid, `${MATERIALS_SHEET_TITLE}!A2:K9999`, [row], token);
          await loadIndex(token, ssid);
        }
        cache.set(key, {...settings});
        console.log(TAG, 'saved', {sheetId, materialKey});
        return;
      }catch(e){
        const msg = (e && e.message) ? e.message : String(e);
        if(/429|quota|RESOURCE_EXHAUSTED/i.test(msg)){
          await sleep(delay); delay = Math.min(delay*1.7, 4000); continue;
        }
        console.warn(TAG, 'save error', e); return;
      }
    }
  }

  function scheduleSave(sheetId, materialKey, settings){
    pending = { sheetId, materialKey, settings };
    if(inFlight) return;
    ...
    saveTimer = setTimeout(async ()=>{
      ...
      inFlight = true;
      try{ await saveNow(p.sheetId, p.materialKey, p.settings); }
      finally{ inFlight = false; if(pending){ scheduleSave(pending.sheetId, pending.materialKey, pending.settings); } }
    }, 500);
  }
  ...
  function collectMaterialsFromScene(scene){
    const out=[];
    if(!scene || !scene.traverse) return [{ key:'GLOBAL', label:'GLOBAL — (all materials)'}];
    scene.traverse(obj=>{
      try{
        if(obj && obj.isMesh){
          const meshName = obj.name || 'Mesh';
    ...
    const m = new Map(); out.forEach(o=>{ if(o.key) m.set(o.key, o); });
    return [{ key:'GLOBAL', label:'GLOBAL — (all materials)'}].concat(Array.from(m.values()));
  }
  function populateTarget(cands){
    const sel = document.getElementById('mat-target'); if(!sel) return false;
    const prev = sel.value; sel.innerHTML = '';
  ...
  function onUIChanged(){
    ...
    const merged = Object.assign({}, DEFAULTS, s);
    cache.set(keyOf(sid, s.materialKey), merged);
    notifyApply(s.materialKey, merged);
    scheduleSave(sid, s.materialKey, merged);
  }
  ...
  async function bootOnce(){
    ...
    console.log(TAG, 'bootOnce');
    let waited = 0;
    while((!window.currentSpreadsheetId || !getActiveSheetId()) && waited < 15000){ await sleep(250); waited += 250; }
    console.log(TAG, 'ids', {spreadsheet: window.currentSpreadsheetId||null, sheetId: getActiveSheetId(), waited: Math.round(waited/250)});

    const token = await ensureAuth(); const ssid = window.currentSpreadsheetId;
    if(token && ssid){ await ensureMaterialsSheet(token, ssid); await loadIndex(token, ssid); }

    let tries=0, cands=null;
    while(tries++ < 60){
      const sc = detectScene();
      const list = collectMaterialsFromScene(sc);
      if(list && list.length){ cands=list; if(populateTarget(cands)) break; }
      await sleep(250);
    }

    wireUI();
    const sel = document.getElementById('mat-target'); const mk = sel?.value || 'GLOBAL';
    const seed = cache.get(keyOf(getActiveSheetId(), mk)) || DEFAULTS;
    writeUI(seed); notifyApply(mk, seed);
  }

  if(document.readyState === 'loading'){ document.addEventListener('DOMContentLoaded', bootOnce, { once:true }); } else { setTimeout(bootOnce, 0); }
  window.addEventListener('materials:refresh', ()=>{ setTimeout(bootOnce, 0); });
})();
// === LM-MATERIALS-PATCH-END ===
