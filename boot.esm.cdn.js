// boot.esm.cdn.js — LociMyu boot (A–E features restored, clean full build)
...
// ---------- Palette & helpers (idempotent where possible) ----------
export const LM_PALETTE = (globalThis.LM_PALETTE ||= ["#ef9368","#e9df5d","#a8e063","#8bb6ff","#b38bff","#86d2c4","#d58cc1","#9aa1a6"]);
export let currentPinColor = (globalThis.currentPinColor ||= LM_PALETTE[0]);
...
export function nearestPalette(hex){
  const p=LM_PALETTE, c=hexToRgb(hex||p[0]); let best=p[0],score=1e9;
  for(const q of p){ const t=hexToRgb(q); const d=(c.r-t.r)**2+(c.g-t.g)**2+(c.b-t.b)**2; if(d<score){score=d;best=q;} }
  return best;
}
function canonicalColor(hex){ try{ return nearestPalette(hex||currentPinColor||LM_PALETTE[0]); }catch{ return hex||LM_PALETTE[0]; } }
...
function ensureIndex(){
  ...
  return getValues(currentSpreadsheetId, "'"+currentSheetTitle+"'!A1:Z9999", token).then(values=>{
    ...
    currentHeaderIdx = {};
    headers.forEach((h,i)=> currentHeaderIdx[h.toLowerCase()] = i);
...
function enrichRow(row){
  ...
  if(row.imageFileId){ p = getFileThumbUrl(row.imageFileId, token, 256).catch(()=> ''); }
...
function updateCaptionForPin(id, fields){
  if(fields && 'color' in fields){ fields.color = canonicalColor(fields.color); }
  ...
  return ensureRow(id, seed).then(()=> ensureIndex()).then(()=>{
    ...
    if(!meta && currentSpreadsheetId){
      ...
      const headers = ['id','title','body','color','x','y','z','imageFileId','createdAt','updatedAt'];
...
function loadCaptionsFromSheet(){
  ...
  getValues(currentSpreadsheetId, range, token).then(values=>{
    ...
    clearPins();
    if(!values.length) { applyColorFilter(); return; }
    const headers = values[0].map(v=> textOrEmpty(v).trim());
    ...
    for(let r=1;r<values.length;r++){
      ...
      const obj = {
        ...
        color: canonicalColor(textOrEmpty(row[map['color']||3]) || currentPinColor),
    ...
    applyColorFilter(); // reflect current filter mode
...
function renderColorChips(){
  const host = $('pin-colors') || $('pin-picker') || $('pinColorChips'); if(!host) return;
  host.innerHTML='';
  LM_PALETTE.forEach(hex=>{
    const b=document.createElement('button');
    b.className='chip chip-color'; b.style.setProperty('--chip', hex); b.title=hex;
    if(nearestPalette(currentPinColor)===hex) b.classList.add('is-active');
    b.addEventListener('click', ()=> setPinColor(hex));
    host.appendChild(b);
  });
}
function saveFilterSet(set){
  try{ localStorage.setItem('lmFilterColors', JSON.stringify(Array.from(set||[]))); }catch(_){}
}
function loadFilterSet(){
  try{
    const saved=JSON.parse(localStorage.getItem('lmFilterColors')||'[]');
    return new Set(saved.length?saved:LM_PALETTE);
  }catch(_){ return new Set(LM_PALETTE); }
}
let filterSet = loadFilterSet();
function renderFilterChips(){
  const host = $('pin-filter') || $('pinFilterChips'); if(!host) return;
  // actions bar (All / None)
  if(!host.previousElementSibling || !host.previousElementSibling.classList || !host.previousElementSibling.classList.contains('chip-actions')){
    ...
    const a=document.createElement('button'); a.className='chip-action'; a.textContent='All';
    const n=document.createElement('button'); n.className='chip-action'; n.textContent='None';
    a.addEventListener('click', ()=>{ filterSet=new Set(LM_PALETTE); saveFilterSet(filterSet); applyColorFilter(); renderFilterChips(); });
    n.addEventListener('click', ()=>{ filterSet=new Set(); saveFilterSet(filterSet); applyColorFilter(); renderFilterChips(); });
  ...
  LM_PALETTE.forEach(hex=>{
    ...
    if(filterSet.has(hex)) b.classList.add('is-on');
    b.addEventListener('click', ()=>{ if(filterSet.has(hex)) filterSet.delete(hex); else filterSet.add(hex); saveFilterSet(filterSet); applyColorFilter(); renderFilterChips(); });
    host.appendChild(b);
  });
}
function rowPassesColorFilter(row){
  if(!row) return false; if(filterSet.size===0) return true;
  return filterSet.has(nearestPalette(row.color||LM_PALETTE[0]));
}
export function applyColorFilter(){
  // list
  const listHost=$('caption-list');
  if(listHost){
  ...
  rowCache.forEach((row, id)=>{
    if(rowPassesColorFilter(row)) addPinMarker({ id, x:row.x, y:row.y, z:row.z, color:row.color||currentPinColor });
  });
  try{ document.dispatchEvent(new CustomEvent('pinFilterChange',{ detail:{ selected:Array.from(filterSet) } })); }catch(_){}
}
export function setPinColor(hex){
  const canon = canonicalColor(hex);
  currentPinColor = canon; globalThis.currentPinColor = canon;
  const host=$('pin-colors')||$('pin-picker')||$('pinColorChips');
  if(host){ host.querySelectorAll('.chip-color').forEach(el=> el.classList.toggle('is-active', getComputedStyle(el).getPropertyValue('--chip').trim()===canon)); }
  if(selectedPinId){
    ...
    try{ updateCaptionForPin(selectedPinId,{ color:canon }); }catch(_){}
  }
}
...
onCanvasShiftPick((x,y,z)=>{
  const id = 'p' + Date.now();
  const color = canonicalColor(currentPinColor||LM_PALETTE[0]);
...
if(sheetSel){
  sheetSel.addEventListener('change', (e)=>{
    ...
    // full reset
    removeCaptionOverlay(selectedPinId||'');
    selectedPinId=null;
    clearPins();
    clearCaptionList();
    rowCache.clear(); captionsIndex.clear();
...
console.log('[LociMyu ESM/CDN] clean full build loaded (A–E)');
