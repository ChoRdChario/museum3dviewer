// boot.esm.cdn.js — COMPLETE (selection sync + images attach/detach + overlay image + sheets write)
import {
...
function extractDriveId(input){
  ...
  const bare=s.match(new RegExp('^[A-Za-z0-9_-]{25,}$')); if(bare) return bare[0];
  try{
    const u=new URL(s);
    const q=u.searchParams.get('id'); if(q && new RegExp('^[A-Za-z0-9_-]{25,}$').test(q)) return q;
    const seg=u.pathname.split('/').filter(Boolean); const ix=seg.indexOf('d');
    if(ix!==-1 && seg[ix+1] && new RegExp('^[A-Za-z0-9_-]{25,}$').test(seg[ix+1])) return seg[ix+1];
    const any=(u.href||'').match(new RegExp('[A-Za-z0-9_-]{25,}')); if(any) return any[0];
  }catch(_){}
  const any2=s.match(new RegExp('[A-Za-z0-9_-]{25,}')); return any2?any2[0]:null;
}
...
const captionsIndex=new Map(); const captionDomById=new Map(); const rowCache=new Map(); const overlays=new Map();
...
function showOverlayFor(id){ const d=rowCache.get(id); if(!d) return; __lm_markListSelected(id); try{ setPinSelected(id,true);}catch(_){ } createCaptionOverlay(id,d); }
...
onCanvasShiftPick(async (pt)=>{
  ...
  selectedPinId=id; setPinSelected(id,true); showOverlayFor(id);
  $('caption-title')?.focus();
});
...
async function putValues(spreadsheetId, rangeA1, values, token){ return fetch('https://sheets.googleapis.com/v4/spreadsheets/'+encodeURIComponent(spreadsheetId)+'/values/'+encodeURIComponent(rangeA1)+'?valueInputOption=RAW',{method:'PUT',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({values})}); }
async function appendValues(spreadsheetId, rangeA1, values, token){ return fetch('https://sheets.googleapis.com/v4/spreadsheets/'+encodeURIComponent(spreadsheetId)+'/values/'+encodeURIComponent(rangeA1)+':append?valueInputOption=RAW&insertDataOption=INSERT_ROWS',{method:'POST',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({values})}); }
async function getValues(spreadsheetId, rangeA1, token){ const r=await fetch('https://sheets.googleapis.com/v4/spreadsheets/'+encodeURIComponent(spreadsheetId)+'/values/'+encodeURIComponent(rangeA1),{headers:{Authorization:'Bearer '+token}}); if(!r.ok) throw new Error('values.get '+r.status); const d=await r.json(); return d.values||[]; }
...
function appendCaptionItem(row){
  ...
  div.appendChild(del);
  div.addEventListener('click', ()=>{ selectedPinId=row.id; __lm_markListSelected(row.id); __lm_fillFormFromCaption(row.id); showOverlayFor(row.id); });
...
async function savePinToSheet(obj){
  ...
  try{ const existed=await getValues(currentSpreadsheetId,"'"+sheetTitle+"'!A1:Z1", token); currentHeaders=(existed[0]||[]).map(h=>(h||'').toString().trim()); currentHeaderIdx={}; currentHeaders.forEach((h,i)=> currentHeaderIdx[h.toLowerCase()]=i); const lower=currentHeaders.map(h=>h.toLowerCase()); if(!(lower.includes('title')&&lower.includes('body')&&lower.includes('color'))) await putValues(currentSpreadsheetId,"'"+sheetTitle+"'!A1:Z1",[LOCIMYU_HEADERS], token);}catch(_){}
...
async function updateImageForPin(id, fileId){ await updateCaptionForPin(id,{ imageFileId:fileId }); }
...
async function loadCaptionsFromSheet(){
  ...
  for(let r=1;r<values.length;r++){
    ...
    const enriched=await enrichRow(obj); appendCaptionItem(enriched); addPinMarker({ id:enriched.id, x:enriched.x, y:enriched.y, z:enriched.z, color:enriched.color }); captionsIndex.set(enriched.id,{rowIndex:r+1});
  }
}
...
function __lm_fillFormFromCaption(id){ const row=rowCache.get(id); const t=$('caption-title'), b=$('caption-body'); if(t) t.value=row?(row.title||''):''; if(b) b.value=row?(row.body||''):''; }
$('caption-list')?.addEventListener('click',(e)=>{ const item=e.target?.closest?.('.caption-item'); if(!item) return; const id=item.dataset.id; if(id){ selectedPinId=id; __lm_markListSelected(id); __lm_fillFormFromCaption(id); showOverlayFor(id);} });

(function(){ const t=$('caption-title'), b=$('caption-body'); let h=null; function q(){ if(h) clearTimeout(h); h=setTimeout(async ()=>{ if(!selectedPinId) return; try{ await updateCaptionForPin(selectedPinId,{ title:t?t.value:'', body:b?b.value:'' }); }catch(e){ console.warn('[caption autosave failed]',e); } }, 500); } t&&t.addEventListener('input', q); b&&b.addEventListener('input', q); })();

console.log('[LociMyu ESM/CDN] boot overlay-edit+fixed-zoom build loaded (完全版+image-fix)');
