...
function renderImageThumb(file){
  ...
  d.className = 'thumb';
  d.style.backgroundImage = `url(${file.thumbUrl})`;
  ...
  bx.className = 'btn-x';
  bx.textContent = '×';
  bx.title = 'Detach image from caption';
  bx.addEventListener('click', (e)=>{
    e.stopPropagation();
    if(!selectedPinId) return;
    updateCaptionForPin(selectedPinId, { imageFileId: '' })
      .then(()=>{
        document.querySelectorAll('#images-grid .thumb.is-active').forEach(t=> t.classList.remove('is-active'));
        const current = document.getElementById('currentImageThumb');
        if(current){ current.innerHTML = '<div class="placeholder">No Image</div>'; }
      })
      .catch(err=> console.warn('[detach image failed]', err));
  });
  d.appendChild(bx);
  ...
  d.addEventListener('click', ()=>{
    ...
    updateCaptionForPin(selectedPinId, { imageFileId: file.id })
      .then(()=>{
        document.querySelectorAll('#images-grid .thumb').forEach(t=> t.classList.remove('is-active'));
        d.classList.add('is-active');
        const cur = document.getElementById('currentImageThumb');
        if(cur){
          cur.innerHTML = '';
      ...
      .catch(err=> console.warn('[attach image failed]', err));
  });

  return d;
}

function populateImagesGrid(files){
