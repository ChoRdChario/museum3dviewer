...
function setPinColor(hex){ window.currentPinColor=hex; const host=document.getElementById('pin-colors'); if(host){ host.querySelectorAll('.chip-color').forEach(el=> el.classList.toggle('is-active', getComputedStyle(el).getPropertyValue('--chip').trim()===hex)); } if(window.selectedPinId && window.rowCache){ const row=rowCache.get(selectedPinId)||{id:selectedPinId}; row.color=hex; rowCache.set(selectedPinId,row); try{ window.refreshPinMarkerFromRow && refreshPinMarkerFromRow(selectedPinId); }catch(_){} try{ window.updateCaptionForPin && updateCaptionForPin(selectedPinId,{ color:hex }); }catch(_){} } }
