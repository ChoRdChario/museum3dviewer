...
// ===== LociMyu Materials: Self-contained module (GV/PV/AV fallbacks, robust init) =====
(function(){
  ...
  async function bootOnce(){
    ...
    let tries = 0; while((!window.currentSpreadsheetId || !getActiveSheetId()) && tries < 60){ await new Promise(r=> setTimeout(r, 250)); tries++; }
    await ensureMaterialsIndex();
    // populate target
    let sTries = 0; while(true){
...
;(() => {
  try {
    ...
    // 1) Kill invisible click-blockers
    $$('div,section').forEach(el => {
      const st = getComputedStyle(el);
      const blocks = (st.position === 'fixed' || st.position === 'absolute') &&
                     parseInt(st.zIndex||'0',10) >= 9999 &&
                     (parseInt(st.width||'0',10) >= window.innerWidth*0.9) &&
                     (parseInt(st.height||'0',10) >= window.innerHeight*0.9);
      if (blocks && st.pointerEvents !== 'none') {
        el.style.pointerEvents = 'none';
      }
    });

    // 2) Ensure tabs clickable
    const tabs = $$('#tabs button,[data-tab]');
    const panes = $$('#panes > *,[data-pane]');
    if (tabs.length && panes.length) {
      tabs.forEach(btn => {
        btn.addEventListener('click', () => {
          const key = (btn.dataset.tab || btn.textContent || '').trim().toLowerCase();
          tabs.forEach(b => b.classList.toggle('active', b === btn));
          panes.forEach(p => {
            const pk = (p.dataset.pane || p.id || '').trim().toLowerCase();
            p.style.display = (pk && key && (pk.includes(key) || key.includes(pk))) ? '' : 'none';
          });
        }, { passive: true });
      });
    }

    // 3) Re-enable UI controls for testing even if not signed
    const enable = (el) => { try { el.disabled = false; el.classList.remove('disabled'); } catch {} };
    ['button','input','select'].forEach(tag => $$(tag).forEach(enable));

    // 4) Materials UI → broadcast change
    ...
    ids.forEach(id => {
      ...
      el.addEventListener(evt, () => {
        const payload = {
          materialKey: $('#mat-target')?.value || 'GLOBAL',
          settings: {
            unlit: !!$('#mat-unlit')?.checked,
            doubleSided: !!$('#mat-doubleside')?.checked,
            opacity: Number($('#mat-opacity')?.value ?? 1),
            white2alpha: !!$('#mat-white2alpha')?.checked,
            whiteThr: Number($('#mat-white-thr')?.value ?? 0.92),
            black2alpha: !!$('#mat-black2alpha')?.checked,
            blackThr: Number($('#mat-black-thr')?.value ?? 0.08),
          }
        };
        window.dispatchEvent(new CustomEvent('materials:ui-change', { detail: payload }));
        if (typeof window.materialsApplyHook === 'function') {
          window.materialsApplyHook(payload);
        }
      }, { passive: true });
    });

    console.log('[LM hotfix] UI unblocked and wired');
  } catch (e) {
    console.warn('[LM hotfix] failed', e);
  }
})();