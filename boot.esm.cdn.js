...
function appendCaptionItem(row){
  ...
  detach.addEventListener('click', (e)=>{ e.stopPropagation(); deleteCaption(row.id); });
...
function updateImageForPin(id, fileIdOrNull){
  ...
  return updateCaptionForPin(id, patch).then(()=>{ try{ renderCurrentImageThumb(); }catch(_){};
    // right-pane preview
...
(function wireForm(){
  ...
  if(t) t.addEventListener('input', function(){ reflectOverlayImmediate(); schedule(); });
  if(b) b.addEventListener('input', function(){ reflectOverlayImmediate(); schedule(); });
})();
...
// --- Hide legacy buttons (+ Pin / Clear / 画像を添付 / デタッチ) ---
document.addEventListener('DOMContentLoaded', ()=>{
  try{
    const texts = ['+ Pin','Clear','画像を添付','デタッチ'];
