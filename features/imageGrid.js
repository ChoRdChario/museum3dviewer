  ...
  function render(){
    ...
    (store.state.images||[]).forEach(file=>{
      ...
      btn.addEventListener('click', async ()=>{
        const sel=store.state.selected; if(!sel) return;
        const pin=store.state.pins.find(p=>p.id===sel); if(!pin) return;
        try{
          if(!toJpegUrlIfNeeded){ const mod=await import('../app/drive_images.js'); toJpegUrlIfNeeded=mod.toJpegUrlIfNeeded; }
          const url=await toJpegUrlIfNeeded(file);
          pin.caption.img=url;
          pin.caption.imageId=file.id;
          pin.caption.thumbnailLink=file.thumbnailLink;
