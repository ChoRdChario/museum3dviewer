// features/auth.js  (v6.6.4)
// Visible Sign in/Sign out UI in three places:
//  - Floating chip (top-right)
//  - Sidebar chip (inside #side)
//  - Inline chip right next to the "LociMyu" title (auto-insert if #auth-inline is absent)
// Also exports ensureLoaded/initAuthUI/signIn/signOut/isSignedIn.
// Works with GIS (accounts.google.com) + GAPI (apis.google.com).
...
export async function ensureLoaded() {
  // UI は出す。GIS/GAPI は遅延で用意してサインイン時に使う。
...
export function signIn(prompt = 'consent') {
  ...
  try { tokenClient?.requestAccessToken({ prompt }); } catch (e) { console.warn('[auth] requestAccessToken failed', e); }
}
...
// ===== UI Rendering (floating + sidebar + inline near title) =====
...
// Try to mount next to the brand title automatically.
// Priority: #auth-inline slot > element with [data-brand] > .brand > h1 containing "LociMyu".
function renderInlineChip() {
  let slot = document.getElementById('auth-inline');
  if (!slot) {
    const findBrand = () => {
      return document.querySelector('#side [data-brand], #side .brand, #side h1, .brand, h1');
    };
...
window.__LMY_authDebug = () => {
  console.log('[authDebug]', {
    ...
    bar: !!document.getElementById('auth-bar'), side: !!document.getElementById('auth-box-side'), inline: !!document.getElementById('auth-inline')
  });
  renderAuthUIs();
};

// Re-render when asked
